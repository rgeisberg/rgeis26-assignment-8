...
def do_experiments(start, end, step_num):
    ...
    fixed_x_min, fixed_x_max = -2, 5  # Adjust these values as needed
    fixed_y_min, fixed_y_max = -5, 5  # Adjust these values as needed
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        # Store the generated data and model for visualization
        ...
        # coef = model.coef_[0]  # Extract the coefficients for the decision boundary
        ...
        plt.plot(x_vals, y_vals, 'k--', label="Decision Boundary")  # Dashed black line for the boundary
        plt.legend()
        ...
        y_pos_eq = y_max - 0.1 * (y_max - y_min)  # 10% from the top for equation text
        y_pos_margin = y_max - 0.2 * (y_max - y_min)  # 20% from the top for margin text
       ...
       # Position the equation text box with a transparent background
        plt.text(x_pos, y_pos_eq, equation_text, fontsize=10, color="black", ha='left',
                 bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3', alpha=0.7))

        # Position the margin width text box slightly below the equation text
        plt.text(x_pos, y_pos_margin, margin_text, fontsize=10, color="black", ha='left',
                 bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3', alpha=0.7))
